#!/usr/bin/env python3
"""Build minimal MRT Jakarta GTFS zip from stop list (community supplement)."""
...
DEFAULT_STOPS = Path("data/gtfs/mrt_stops.json")
STATIC_STOPS = Path("data/gtfs/static/mrt_stops.json")
...
def build(stops_path: Path, output: Path) -> None:
    ...
    agency = [
        {
            ...
            "agency_url": "https://www.jakartamrt.co.id",
    ...
    route_rows = [
        {
            "route_id": "MRT-NS",
            "agency_id": "MRT",
            "route_short_name": "MRT",
            "route_long_name": "MRT Jakarta North-South Line",
            "route_type": "1",
        }
    ]
    calendar = [
        {
            "service_id": "MRT-WEEKDAY",
            "monday": "1",
            "tuesday": "1",
            "wednesday": "1",
            "thursday": "1",
    ...
    print(f"Built MRT GTFS ({len(stops)} stops) -> {output}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--stops", default=str(DEFAULT_STOPS))
    parser.add_argument("--output", default="data/gtfs/mrt_minimal.zip")
